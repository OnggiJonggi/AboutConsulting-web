...
public class ConsultantService{
	 ...
	 * 컨설턴트 세부사항 조회
	 * 
	 * 1. CONSULTANT, CONSULTANT_ORG 조회
	 * 2. CONSULTANT_STUDENT 조회
	 */
	...
	public void insertCharged(int consultantNo, Set<Integer> studentNos) {
		...
		if(isDupli>0) throw new CustomException(ErrorCodeEnum.CONSULTANT_STUDENT_IS_DUPLICATED);
		
		// 삽입
		int result = consultantMapper.insertCharged(consultantNo, studentNos);
		if(result==0) throw new CustomException(ErrorCodeEnum.FAILED_CREATE_CONSULTANT_STUDENT);
	}
	...
	public void deleteCharged(int consultantNo, int studentNo) {
		...
		if(result==0) throw new CustomException(ErrorCodeEnum.FAILED_DELETE_CONSULTANT_STUDENT);
	}
